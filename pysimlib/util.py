...
# Please note there is also a utils (note the plural 's') file.
...
def get_img_path(fname):
    img_dir = os.path.join(os.path.dirname(config.conf.mydir), \
                    'pysimlib/images')
...
def get_exec_path(fname):
    exec_dir = os.path.dirname(config.conf.mydir)
    exec_path2 = os.path.join(exec_dir, "pysimlib")
...
# return pos xxx yyy www hhh of current display
