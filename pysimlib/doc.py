...
# SIMULATION specific
from . import logic, decor, draw, control, qcan, radio
...
class Doc(Gtk.DrawingArea, draw.Draw, logic.Logic):
    ...
    def __init__(self, buff, appwin, readonly = False):
        ...
        # Sub classes
        draw.Draw.__init__(self)
        logic.Logic.__init__(self)
        
        # Friends
        #self.decor_1 = decor.Decor(self, 0)
        #self.decor_2 = decor.Decor(self, 1)
        #self.decor_3 = decor.Decor(self, 2)
        #self.decor_4 = decor.Decor(self, 3)
    
        #self.qcan_1 = qcan.Qcan(self, 0)
        #self.qcan_2 = qcan.Qcan(self, 1)
        #self.qcan_3 = qcan.Qcan(self, 2)
        #self.qcan_4 = qcan.Qcan(self, 3)

        self.control = control.Control(self)
        self.radio  = radio.Radio(self)
        
        #self.qlcd = qlcd.QcanLCD( self.decor_1, self.decor_2, self.decor_3, self.decor_4)
        
        # Our font
        fsize  =  config.conf.sql.get_int("fsize")
        fname  =  config.conf.sql.get_str("fname")
        if fsize == 0: fsize = 14
        ...
        #self.connect("unmap", self.OnClose)
        #self.connect("delete-event", self.area_destroy)    
        
        #print("Created ", self)
    
    def close_subwins(self):
    
        #print ("Close subwins")
        # Force calling all the decor's destructors:
        #self.decor_1.close()
        #self.decor_2.close()
        #self.decor_3.close()
        #self.decor_4.close()
    ...
    def setcol(self):        
        ccc = config.conf.sql.get_str("fgcolor")
        if ccc == "":
            self.fgcolor  = color.str2float(FGCOLOR)              
        else: 
            self.fgcolor  = color.str2float(ccc)
        #print "fgcol", self.fgcolor, ccc
                     
        ccc = config.conf.sql.get_str("rbgcolor")
        if ccc == "":
            self.rbgcolor = color.str2float(RBGCOLOR)              
        else: 
            self.rbgcolor = color.str2float(ccc)
        #print "rgbcolor", self.rbgcolor, ccc
        
        ccc = config.conf.sql.get_str("cbgcolor")
        if ccc == "":
            self.cbgcolor = color.str2float(CBGCOLOR)              
        else: 
            self.cbgcolor = color.str2float(ccc)
        #print "cbgcolor", self.cbgcolor, ccc
        
        ccc = config.conf.sql.get_str("kwcolor")
        if ccc == "":
            self.kwcolor = color.str2float(KWCOLOR)              
        else: 
            self.kwcolor = color.str2float(ccc)
        #print "load kwcolor", self.kwcolor, ccc
        
        ccc = config.conf.sql.get_str("clcolor")
        if ccc == "":
            self.clcolor = color.str2float(CLCOLOR)              
        else: 
            self.clcolor = color.str2float(ccc)
        
        ccc = config.conf.sql.get_str("cocolor")
        if ccc == "":
            self.cocolor = color.str2float(COCOLOR)              
        else: 
            self.cocolor = color.str2float(ccc)
       
        ccc = config.conf.sql.get_str("stcolor")
        if ccc == "":
            self.stcolor = color.str2float(STCOLOR)              
        else: 
            self.stcolor = color.str2float(ccc)
    ...
    def idle_callback(self):
        ...
        try:
            if self.changed:
                ...
                xfile = config.conf.data_dir + "/" + hhh
    ...
    def grab_focus_cb(self, widget):
        #print ("doc: grab_focus_cb", widget)
        pass
    ...
    def configure_event(self, widget, event):
        #print "configure_event", event.type
        #print "type", event.width, event.height
        if not self.oldrect:
            self.oldrect = self.get_allocation()
        newsize = self.get_allocation()
        if newsize.width != self.oldrect.width or \
                newsize.height != self.oldrect.height:
            self.init_poks()
            self.init_agvs(True)
    ...
    def area_button(self, area, event):
        ...
        if  event.type == Gdk.EventType.BUTTON_PRESS:
            ...
            if event.state & Gdk.ModifierType.CONTROL_MASK:
                config.conf.keyh.act.alt_l(self)
            elif event.state & Gdk.ModifierType.SHIFT_MASK:
                config.conf.keyh.act.ctrl_b(self)
            else: 
                config.conf.keyh.act.alt_v(self)
        else:
    ...
    def update_bar2(self):
        try: 
            clip = config.conf.keyh.act.currclip;
    ...
    def area_key(self, area, event):
        ...
        #print "area_key", event.keyval, event.type
        ...
        # Restart timer ticks
        config.conf.idle = config.conf.IDLE_TIMEOUT
        config.conf.syncidle = config.conf.SYNCIDLE_TIMEOUT
    ...
    def addict(self, widget, string, arg):
        #print "addict", arg
        lw = arg.lower()
        xfile = config.conf.config_dir + "/" + "userdict.txt"
        try:
            fd = open(xfile, "a+")                 
        except:
    ...
    def menuitem_response(self, widget, string, arg):
        ...
        config.conf.keyh.act.clip_cb(None, string, self)
        self.fired += 1
        GLib.timeout_add(300, self.keytime)
    ...
    def loadparms(self):
        ...
        self.startxxx  =  config.conf.sql.get_int(hhh + "/xx")
        self.startyyy  =  config.conf.sql.get_int(hhh + "/yy")
        # Note: we set cursor on first focus
        
    # Save per file parms (cursor, fname, etc)         
    def  saveparms(self):
        hhh = hash_name(self.fname)           
        config.conf.sql.put(hhh + "/xx", self.xpos + self.caret[0])        
        config.conf.sql.put(hhh + "/yy", self.ypos + self.caret[1])        
        config.conf.sql.put(hhh + "/fname", self.fname)        
   ...
   # Create backup
    ...
    def savemacro(self):
        ...
        fname = "untitled.mac"
        xfile = config.conf.config_dir + "/macros/" + fname        
        old = os.getcwd()
        try:
    ...
    def loadmacro(self):
        ...
        xfile = config.conf.config_dir + "/macros/"    
        old = os.getcwd()
        try:
            os.chdir(os.path.dirname(xfile))
        except:
