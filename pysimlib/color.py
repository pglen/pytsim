...
def colors(self, self2):
    ...
    try:
        dialog.set_icon_from_file(get_img_path("pyagvsim_sub.png"))
    except:
...
def col_one(butt, title, ev, self2):
    ...
    config.conf.sql.put("fgcolor", float2str(ev.color))
...
def col_two(butt, title, ev, self2):
    ...
    config.conf.sql.put("rbgcolor", float2str(ev.color))
    for mm in range(self2.notebook.get_n_pages()):
        vcurr = self2.notebook.get_nth_page(mm)
        vcurr.area.setcol()
    self2.invalidate()
...
def col_three(butt, title, ev, self2):
    ...
    config.conf.sql.put("cbgcolor", float2str(ev.color))
    for mm in range(self2.notebook.get_n_pages()):
        vcurr = self2.notebook.get_nth_page(mm)
        vcurr.area.setcol()
    self2.invalidate()
...
def col_four(butt, title, ev, self2):
    ...
    config.conf.sql.put("kwcolor", float2str(ev.color))        
    for mm in range(self2.notebook.get_n_pages()):
        vcurr = self2.notebook.get_nth_page(mm)
        vcurr.area.setcol()
    self2.invalidate()
...
def col_five(butt, title, ev, self2):
    ...
    config.conf.sql.put("clcolor", float2str(ev.color))
    for mm in range(self2.notebook.get_n_pages()):
        vcurr = self2.notebook.get_nth_page(mm)
        vcurr.area.setcol()
    self2.invalidate()
...
def col_six(butt, title, ev, self2):
    ...
    config.conf.sql.put("cocolor", float2str(ev.color))
    for mm in range(self2.notebook.get_n_pages()):
        vcurr = self2.notebook.get_nth_page(mm)
        vcurr.area.setcol()
    self2.invalidate()
...
def col_seven(butt, title, ev, self2):
    ...
    config.conf.sql.put("stcolor", float2str(ev.color))
    for mm in range(self2.notebook.get_n_pages()):
        vcurr = self2.notebook.get_nth_page(mm)
        vcurr.area.setcol()
    self2.invalidate()
...
def  def_col(butt, self2):
    ...
    self2.fgcolor  = str2float(doc.FGCOLOR)              
    self2.rbgcolor = str2float(doc.RBGCOLOR)              
    self2.cbgcolor = str2float(doc.CBGCOLOR)              
    self2.kwcolor  = str2float(doc.KWCOLOR)
    self2.clcolor  = str2float(doc.CLCOLOR)
    self2.cocolor  = str2float(doc.COCOLOR)
    self2.stcolor  = str2float(doc.STCOLOR)
        
    self2.invalidate()
    
    config.conf.sql.put("fgcolor",  float2str(self2.fgcolor) )       
    config.conf.sql.put("rbgcolor", float2str(self2.rbgcolor))
    config.conf.sql.put("cbgcolor", float2str(self2.cbgcolor))
    config.conf.sql.put("kwcolor",  float2str(self2.kwcolor) )
    config.conf.sql.put("clcolor",  float2str(self2.clcolor) )
    config.conf.sql.put("cocolor",  float2str(self2.cocolor) )
    config.conf.sql.put("stcolor",  float2str(self2.stcolor) )
